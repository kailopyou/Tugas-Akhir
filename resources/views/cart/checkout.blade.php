...
@section('content')
    <div class="">
        ...
        @if (empty(Auth::user()->user_phone) ||
                empty(Auth::user()->user_fullname) ||
                empty(Auth::user()->user_state) ||
                empty(Auth::user()->user_city) ||
                empty(Auth::user()->user_zipcode) ||
            ...
            <form action="{{ route('orders.store') }}" method="post" id="checkoutForm" style="display: none;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="shipping_fullname" id="shipping_fullname" class="form-control"
                        value="{{ old('shipping_fullname', Auth::user()->user_fullname ?? '') }}">
                </div>

                <div class="form-group">
                    <label for="shipping_state">State</label>
                    <input type="text" name="shipping_state" id="shipping_state" class="form-control"
                        value="{{ old('shipping_state', Auth::user()->user_state ?? '') }}">
                </div>

                <div class="form-group">
                    <label for="shipping_city">City</label>
                    <input type="text" name="shipping_city" id="shipping_city" class="form-control"
                        value="{{ old('shipping_city', Auth::user()->user_city ?? '') }}">
                </div>

                <div class="form-group">
                    <label for="shipping_zipcode">Zip</label>
                    <input type="number" name="shipping_zipcode" id="shipping_zipcode" class="form-control"
                        value="{{ old('shipping_zipcode', Auth::user()->user_zipcode ?? '') }}">
                </div>
